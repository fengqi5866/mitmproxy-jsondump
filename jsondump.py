...
class JSONDumper:
    ...
    def dump(self, frame):
        ...
        frame['client_conn']['sockname'] = [str(element) for element in frame['client_conn']['sockname']]
        if self.outfile:
            self.lock.acquire()
            self.outfile.write(json.dumps(frame) + "\n")
            self.lock.release()
        else:
            response = requests.post(self.url, json=frame, auth=(self.auth or None))
            print(response.text)
    ...
    def load(loader):
        ...
        loader.add_option('dump_traceid', str, '',
                          'traceid.')
    ...
    def configure(self, _):
        ...
        self.traceid = ctx.options.dump_traceid
    ...
    def request(self, flow):
        ...
        if isinstance(flow, http.HTTPFlow):
            flow.request.headers.add(self.traceid, flow.id)
